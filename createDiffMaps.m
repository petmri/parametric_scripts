function [adc] = createDiffMaps(filename, b_values, fitype, ploter, ...
                                noisepath, weight)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   createDiffmaps  -   calculate and create ADC, R2 and So maps
%                       using y = So*e^(ADC*x) exponential fitting
%
%   Parameters:     
%       filename  -  input image filename (required)
%       b-values  -  array of b-values (if this parameter is missing,
...
%{
    ...
    adcfilename = strcat(strdir, '/', 'ADC_map_', fitype,'_', strrecon, '.nii')
    ...
    save_nii(nii, sofilename);
        ...
        if(strcmp(fitype, 'ackerman'))
              ...
              sigfilename = strcat(strdir, '/', 'Var_map_',fitype,'_', strrecon, '.nii')
