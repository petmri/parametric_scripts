...
function errorlog = processADC(dateA, subjectA, index, b_val, directions, ...
                               core, affil, varargin) 
  %{
  partial, partindex, ...
 ...
 try
  ...
  inpath = fullfile(core, affil, subjectA, dateA, 'ADC');
  fprintf(['Inpath: ' inpath '\n']);

 
 [f,b,n,tot] = getFiles(inpath, strcat(dateA, '_',subjectA), indext, 'nii');
  ...
  for i = 1:tot
    ...
    if(size(g) ~= 0)
      factora = (numB -1)*numDIR + 1;
    else
        factora = numB*numDIR + 1; 
    end
    factora;
   ...
   numDIR
    ...
    for i = 1:numDIR
      ...
      output = oneDIRADC(imageb, b_val,fitype, noisemat, maskimage, 'mode', status, ...
      ...
      if(separate == 1)
        fullpathADC = fullfile(inpath, ['ADC_map_', fitype,'_', filename,'.nii'])
        ...
        ADCdirnii = make_nii(SADC, res, [], [], b_val);
        Rdirnii   = make_nii(SR, res, [], [], b_val);
      ...
      if(separate ~= 1)
        fullpathADC = fullfile(inpath, ['ADC_map_', fitype,'_all', filename, ...
                            '.nii'])
